	...
	var tree = {
		...
		createNode: function (
				...
				if (p_parentNode == undefined) {
					...
					if (p_parentNode.childNodes.length == 0) {
						if (p_parentNode.expanded) {
							p_parentNode.elementLi.getElementsByTagName(
								"ul"
			...
			if (p_parentNode == undefined) {
				...
				node.parent = this;
			} else p_parentNode.childNodes.push(node);
		...
		drawNode: function (p_ulElement, p_node) {
			...
			if (v_icon != undefined) v_span.appendChild(v_icon);
			...
			if (p_node.childNodes.length > 0) {
				if (!p_node.expanded) v_ul.style.display = "none";
		...
		setText: function (p_node, p_text) {
			p_node.elementLi.getElementsByTagName("span")[0].lastChild.innerHTML =
				p_text;
			p_node.text = p_text;
		},
		...
		toggleNode: function (p_node) {
			if (p_node.childNodes.length > 0) {
				if (p_node.expanded) p_node.collapseNode();
				else p_node.expandNode();
			}
		},
		...
		selectNode: function (p_node) {
			...
			if (this.selectedNode != null && this.selectedNode != p_node)
				this.selectedNode.elementLi.getElementsByTagName("span")[0].className =
					"node";
		...
		removeNode: function (p_node) {
			...
			if (p_node.elementLi.className == "last" && index != 0) {
				...
				p_node.parent.childNodes[index - 1].elementLi.style.backgroundColor =
					this.backcolor;
			}
		...
		nodeContextMenu: function (p_event, p_node) {
			if (p_event.button == 2) {
				...
				if (p_node.contextMenu != undefined) {
					...
					} else v_div = this.contextMenuDiv;
					...
					for (var i = 0; i < v_menu.elements.length; i++)
						(function (i) {
							...
							if (v_menu.elements[i].submenu != undefined) {
								...
								v_span_more.appendChild(
									createImgElement(null, "menu_img", "images/right.png")
								);
			...
			if (tree.childNodes.length > 0) {
				...
				if (p_recursive) {
					for (var i = 0; i < tree.getElementsByTagName("ul").length; i++) {
						...
						if (check_ul.childNodes.length != 0) lists[lists.length] = check_ul;
					}
				}
			}
